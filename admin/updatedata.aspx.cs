...
public partial class admin_updatedata : System.Web.UI.Page
{
        ...
        try
        {
            ...
            using (XLWorkbook workBook = new XLWorkbook(filePath))
            {
                ...
                foreach (IXLRow row in workSheet.Rows())
                {
                    ...
                    else
                    {
                        ...
                        foreach (IXLCell cell in row.Cells())
                        {
                            ...
                            {
                                ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "Notify", "alert('Column A (CCODE) can not be empty.');window.location='dashboard.aspx'", true);
                                return;
                            }
                            if (cell.Address.ColumnLetter == "B" && string.IsNullOrEmpty(cell.Value.ToString()))
                            {
                                ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "Notify", "alert('Column B (Centre Name) can not be empty.');window.location='dashboard.aspx'", true);
                                return;
                            }
                            if (cell.Address.ColumnLetter == "C" && string.IsNullOrEmpty(cell.Value.ToString()))
                            {
                                ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "Notify", "alert('Column C (atccode) can not be empty.');window.location='dashboard.aspx'", true);
                                return;
                            }
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    {
                        SqlCommand sqlCommand2 = new SqlCommand("spUpdate_ExamdataATCCodeExcelData N'" + dr[0].ToString().Replace("'", "''")
                            + "',N'" + dr[1].ToString().Replace("'", "''") + "',N'" + dr[2].ToString().Replace("'", "''")
                            + "'", sqlConnection2);
                ...
                ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "Notify", "alert('Data Updated Successfully.');window.location='updatedata.aspx'", true);
                return;
            }
        }
        catch (Exception ex)
    ...
    protected void btnDownloadUpdatedExcelFormat_Click(object sender, EventArgs e)
    {
        try
        {
            DataSet dataSet = MySql.GetDataSet("spGet_FullCenterDetailsForATCCode");
            gvFullFormatExcelData.DataSource = dataSet;
            gvFullFormatExcelData.DataBind();

            if (dataSet != null && dataSet.Tables.Count > 0 && dataSet.Tables[0].Rows.Count > 0)
            {
                ExportGridToExcel(dataSet, "FormatAiimsDelhiATCCode");
            }
